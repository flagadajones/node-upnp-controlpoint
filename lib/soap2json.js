        ...
        xmlToJSON: function (xdoc) {
            try {
                ...
                function formatName(name) {
                    var regEx = /-/g;
                    var tName = String(name).replace(regEx, "_"); //.replace("[a-z]+:","");
                    return tName;
                }
                ...
                var cleanUp = function (doc) {
                    var keys = Object.keys(doc);

                    for (var i = 0; i < keys.length; i++) {
                        var item = doc[keys[i]];
                        if (keys[i].startsWith("xmlns") || (keys[i] === "Text" && doc[keys[i]] === "") || keys[i] === "RootName" || keys[i] === "typeOf") {
                            doc[keys[i]] = undefined;
                        } else
                        if (item instanceof Object) {
                            if (Array.isArray(item)) {
                                if (item.length == 1 && Object.keys(item[0]).length == 1 && Object.keys(item[0])[0] === "Text") {
                                    doc[keys[i]] = item[0].Text;
                                } else {
                                    for (var j = 0; j < item.length; j++) {
                                        item[j] = cleanUp(item[j]);
                                    }
                                }
                            } else {
                                doc[keys[i]] = cleanUp(item);
                            }
                        }
                    }
                    return doc;
                };
                return cleanUp(tmpObj);
            } catch (e) {
                return null;
            }
        }
...
var extractResponse = function (name, buf) {
    var xmlDoc = domParser.parseFromString(buf, 'text/xml');
    var json = XMLObjectifier.xmlToJSON(xmlDoc);
    
    if(json.Body[0].Fault)
    {    return {Error: json.Body[0].Fault[0].detail[0].UPnPError[0]};
    }
     return json.Body[0][name + 'Response'][0];
}

exports.XMLObjectifier = XMLObjectifier;
