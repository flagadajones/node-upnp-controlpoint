...
var UpnpService = function (device, desc) {
    ...
    this.controlUrl = ("/"+desc.controlURL[0]).replace("//","/");
    this.eventSubUrl = ("/"+desc.eventSubURL[0]).replace("//","/");
    this.scpdUrl = ("/"+desc.SCPDURL[0]).replace("//","/");
...
UpnpService.prototype.initActions = function (completeDesc) {
    var self = this;
    
    if (completeDesc.actionList) {
        completeDesc.actionList[0].action.forEach(function (actionDesc) {

            var action = new UpnpAction(actionDesc);
            if (TRACE) {

                console.log(self.serviceId + " - action:" + action.name);
            }
...
UpnpService.prototype.subscribe = function (callback) {
    ...
    var callbackUrl = "http://" + this.device.controlPoint.eventHandler.localAddress + ":" + this.device.controlPoint.eventHandler.serverPort + "/listener";
    ...
    var req = http.request(options, function (res) {
        ...
        res.on('end', function () {
            if (res.statusCode !== 200) {
                callback(new Error("Problem with subscription on " + JSON.stringify(options)), buf);
            } else {
